...
int main(int argc,char* argv[]) {
    ...
    const std::string filename = argv[1];//"./test_data/test.xyz";
    const std::string o_name = argv[2];
    const double resolution = atof(argv[3]);
...
//    WritePoints(o_name.c_str(), n_supervoxels, points, labels);
...
//     const double voxel_resolution = resolution / 8.;
//
...
//    LOG(INFO) << n_supervoxels << " supervoxels computed.";
//    WritePoints(o_name.c_str(), n_supervoxels, points, vccs_labels);
    ...
    LOG(INFO) << n_supervoxels << " supervoxels computed.";
    WritePoints(o_name.c_str(), n_supervoxels, points, vccs_knn_labels);
